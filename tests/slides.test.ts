...
describe("should have only defined slides", () => {
  it.each(slides)("%s", (slidePath: string) => {
    expect(slidePath).toBeDefined();
    expect(fs.existsSync(path.resolve(path.join(MARKDOWN_PATH, slidePath))))
      .toBeTruthy();
  });
});
...
describe("every lab should point out the correct command to run the exercise", () => {
  it.each(labSlides)("%s", (slidePath) => {
    ...
    expect(
      fs.existsSync(
        path.resolve(
          path.join(STEPS_PATH, command.split("npm run ")[1].trim()),
        ),
      ),
    ).toBeTruthy();
  });
...
describe("every exercise should be pointed out in a lab slide", () => {
  const exercises = (
    JSON.parse(
      fs.readFileSync(
        path.resolve(path.join(STEPS_PATH, "package.json")),
        "utf-8",
      ),
    ).steps as string[]
...
describe("every image found in slide should exists", () => {
  ...
  it.each(imagesInSlide)("%s", (imageInSlide) => {
    expect(fs.existsSync(path.resolve(path.join(DOCS_PATH, imageInSlide))))
      .toBeTruthy();
  });
});
describe("every image found in image folder should be in a slide", () => {
  const imageFilesPath = fs
    ...
    .filter((imagePath) =>
      !fs.statSync(path.resolve(path.join(IMAGES_PATH, imagePath)))
        .isDirectory()
    )
    .filter((imagePath) =>
      !imagePath.includes("sfeir-school-logo.png") &&
      !imagePath.endsWith(".txt")
    );
  const allMdFilesConcat = allMdFiles.map((md) => md.md).join("\n\n\n\n\n\n");
  it.each(
    imageFilesPath.map((p) => path.join(IMAGES_PATH, p)).map((p) =>
      p.replace("../docs/", "")
    ),
  )(
    "%s",
...
describe("only valid class should be used", () => {
  const cssFiles = [
    ...
    path.resolve(
      path.join(
        DOCS_PATH,
        "web_modules/sfeir-school-theme/fontello-sfeir/css/sfeir-codes.css",
      ),
    ),
    path.resolve(
      path.join(
...
const slides: string[] = (await import(getSlidesJsImport())).formation().map((
  slide,
) => slide.path);
const labSlides = slides.filter((path) =>
  path.includes("-lab-") || path.includes("-lab.")
);
...
function readMd(mdPath: string) {
  return fs.readFileSync(
    path.resolve(path.join(MARKDOWN_PATH, mdPath)),
    "utf-8",
  );
}
